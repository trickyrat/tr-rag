...
class RAGSystem:
    ...
    def __init__(self, config: Optional[RAGConfig] = None):
        ...
        Args:
            config: RAG config, if None uses DEFAULT_CONFIG
        """
        ...
        # TODO: Initialize GenerationIntegration with actual LLM model and parameters
        # logger.info("🤖 Initializing Generation Integration...")
        # self.generation = GenerationIntegration(
    ...
    def build_knowledge_base(self):
        ...
        # 1. 尝试加载已保存的索引
        vectorstore = self.vector_builder.load_index()
        ...
        if vectorstore is not None:
            ...
            # 5. 保存索引
            print("保存向量索引...")
            self.vector_builder.save_index()

        # 6. 初始化检索优化模块
        print("初始化检索优化...")
        self.retrieval = RetrievalOptimization(vectorstore, chunks)

        # 7. 显示统计信息
        stats = self.doc_chunker.get_statistics()
        print("\n📊 知识库统计:")
    ...
    def ask_question(self, question: str, stream: bool = False):
        ...
        # 1. 查询路由
        route_type = self.generation.query_router(question)
        ...
        if route_type == "list":
            ...
            # 详细查询和一般查询使用智能重写
            print("🤖 智能分析查询...")
            rewritten_query = self.generation.query_rewrite(question)
        ...
        if filters:
            ...
            relevant_chunks = self.retrieval.metadata_filtered_search(
                rewritten_query, filters, top_k=self.config.top_k
            )
        else:
            relevant_chunks = self.retrieval.hybrid_search(
        ...
        if route_type == "list":
            # 列表查询：直接返回菜品名称列表
            print("📋 生成菜品列表...")
            relevant_docs = self.data.get_parent_documents(relevant_chunks)
            ...
            return self.generation.generate_list_answer(question, relevant_docs)
        else:
            # 详细查询：获取完整文档并生成详细回答
            print("获取完整文档...")
            relevant_docs = self.data.get_parent_documents(relevant_chunks)
            ...
            if route_type == "detail":
                # 详细查询使用分步指导模式
                if stream:
                    return self.generation.generate_step_by_step_answer_stream(
                        question, relevant_docs
                    )
                else:
                    return self.generation.generate_step_by_step_answer(
                        question, relevant_docs
                    )
            else:
                # 一般查询使用基础回答模式
                if stream:
                    return self.generation.generate_basic_answer_stream(
                        question, relevant_docs
                    )
                else:
                    return self.generation.generate_basic_answer(
                        question, relevant_docs
                    )
    ...
    def _extract_filters_from_query(self, query: str) -> dict:
        ...
        filters = {}
        # 分类关键词
        category_keywords = DataPreparation.get_supported_categories()
        ...
        # 难度关键词
        difficulty_keywords = DataPreparation.get_supported_difficulties()
    ...
    def search_by_category(self, category: str, query: str = "") -> List[str]:
        ...
        docs = self.retrieval.metadata_filtered_search(search_query, filters, top_k=10)
    ...
    def get_ingredients_list(self, dish_name: str) -> str:
        ...
        # 搜索相关文档
        docs = self.retrieval.hybrid_search(dish_name, top_k=3)

        # 生成食材信息
        answer = self.generation.generate_basic_answer(
