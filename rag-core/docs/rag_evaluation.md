...
flowchart LR
    ...
    C --> D[src/scripts/convert_testset_to_hash.py<br/>转换测试集为 content_hash 模式]
...
uv run convert_testset_to_hash
...
| 脚本                                     | 用途                                                                                       |
| ---------------------------------------- | ------------------------------------------------------------------------------------------ |
| `src/scripts/dump_chunks.py`             | 按文档分组打印所有 chunk 的 ID 和内容预览，用于人工编写查询时参考                          |
| `src/scripts/generate_testset.py`        | 基于 `chunk_mapping.jsonl` 批量生成覆盖所有知识库文档的测试用例                            |
| `src/scripts/remap_testset.py`           | 将测试集中的旧 chunk_id 替换为当前 chunk_id（chunk_id 模式专用）                           |
| `src/scripts/convert_testset_to_hash.py` | **推荐**：将测试集从 `relevant_chunk_ids` 转换为 `relevant_content_hashes`（内容哈希模式） |
...
```python
from evaluation.testset_builder import TestSetBuilder
...
import logging
from config import DEFAULT_CONFIG
from rag import DocumentChunker, VectorStoreBuilder, RetrievalOptimization
